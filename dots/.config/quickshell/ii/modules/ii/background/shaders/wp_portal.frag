...
void main() {
    ...
    vec2 acUv     = vec2(uv.x * ubuf.aspectRatio.x, uv.y);
    vec2 acCenter = vec2(center.x * ubuf.aspectRatio.x, center.y);
    float dist    = length(acUv - acCenter);

    // Max radius from center to cover screen
    float maxDistX = max(center.x * ubuf.aspectRatio.x, (1.0 - center.x) * ubuf.aspectRatio.x);
