...
layout(std140, binding = 0) uniform buf {
    ...
    vec2  aspectRatio;   // Width / Height of the screen
...
void main() {
    ...
    vec2 adjustedUV = vec2(uv.x * ubuf.aspectRatio.x, uv.y);
    vec2 adjustedCenter = vec2(ubuf.centerX * ubuf.aspectRatio.x, ubuf.centerY);
    ...
    float maxDistX = max(ubuf.centerX * ubuf.aspectRatio.x,
                         (1.0 - ubuf.centerX) * ubuf.aspectRatio.x);
    ...
    float adjustedSmoothness = mappedSmoothness * max(1.0, ubuf.aspectRatio.x);
