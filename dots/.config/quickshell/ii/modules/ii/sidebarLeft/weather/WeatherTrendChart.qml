...
Canvas {
    ...
    property color secondLineColor: Appearance.colors.colSecondary ?? "#a08cb5"
    ...
    property color textColor: Appearance.colors.colOnSurfaceVariant ?? Appearance.m3colors.m3onSurfaceVariant
    ...
    onPaint: {
        ...
        const usableW = width - padX * 2;
        const usableH = height - padY * 2;
        const xAt = i => padX + usableW * i / (count - 1);
        const yAt = value => padY + usableH * (1 - (value - min) / (max - min));
        ...
        ctx.font = "11px '" + (Appearance.font.family.numbers ?? "sans-serif") + "', sans-serif";
        ctx.textAlign = "center";
        for (let n = 0; n < count; ++n) {
            if (n !== 0 && n !== count - 1 && n % 2 !== 0)
                continue;

            const temp = Weather.useUSCS ? Math.round(values[n] * 9 / 5 + 32) : Math.round(values[n]);
            ctx.fillText(temp + "°", xAt(n), Math.max(11, yAt(values[n]) - 8));
        }
    }
