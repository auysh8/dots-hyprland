...
MouseArea {
    ...
    function updateCenterHighlight() {
        ...
        if (canvas.setCenterActive) canvas.setCenterActive(nearX, nearY)
    }
    ...
    onDraggingChanged: {
        var canvas = findCanvas(root.parent)
        if (canvas && canvas.setDragging) canvas.setDragging(dragging)
        ...
        if (!dragging && canvas) {
            ...
            if (Config.options && Config.options.background && Config.options.background.showSnapLines && canvas.flashLines)
