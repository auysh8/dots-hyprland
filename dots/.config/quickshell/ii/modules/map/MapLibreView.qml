...
Item {
    ...
    function reload() {
        ...
        request.onreadystatechange = function () {
            ...
            if (request.status < 200 || request.status >= 300) {
                root.fail(Translation.tr("Map style temporarily unavailable"));
                return;
            }
            try {
                JSON.parse(request.responseText);
            } catch (error) {
                root.fail(Translation.tr("Invalid map style"));
                return;
            }
    ...
    function fail(message) {
        mapLoader.active = false;
        root.mapState = "error";
        root.errorMessage = message || Translation.tr("Map temporarily unavailable");
    }
    ...
    Loader {
        ...
        onStatusChanged: {
            if (status === Loader.Error) {
                root.fail(Translation.tr("Unable to create map"));
    ...
    MapFallback {
        ...
        message: root.errorMessage.length > 0 ? root.errorMessage : Translation.tr("Map temporarily unavailable")
