...
Scope {
    ...
    function transparentize(color, alpha) { return Qt.rgba(color.r, color.g, color.b, alpha) }
    function getBgColor() {
        if (root.popupType === "bad") return root.bgBad
        if (root.popupType === "good") return root.bgGood
        if (root.isBottomPopup) return transparentize(root.themeColors.m3surfaceContainerHighest, 0.75)
    ...
    Timer {
        ...
        onTriggered: { root.displayState = "hidden" }
    }
    LazyLoader {
        id: popupLoader
        ...
        PanelWindow {
            ...
            Rectangle {
                ...
                height: {
                    if (root.displayState === "indicator") return layout.implicitHeight + 20
